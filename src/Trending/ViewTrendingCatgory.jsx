...
export default function ViewTrendingCatgory() {
  ...
  const Remove = async (item) => {
    try {
      const response = await axios.post("http://localhost:9090/removecategoryfromtrending", {
        id: item.id,
        name: item.name
      })
      alert(response.data)
  ...
  const Search = async (e) => {
    ...
    try {
      if (name === null || name === undefined || name === "" || name === " " || name.length === 0) {
        ...
        const response = await axios.post("http://localhost:9090/viewtrendingcat", {
          name: name,
        })
        setCat(response.data)
      }
    ...
    <div className='m-10'>
      ...
      <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-2">
        {(items) ?
                ...
                cat?.map((c) => (
                  <tr key={c.id} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="p-4 flex justify-center">
                      {
                        (c.image === null || c.image === undefined || c.image.startsWith("http") === false) ? <img src="https://img.freepik.com/free-vector/no-data-concept-illustration_114360-626.jpg" class="w-16 md:w-32 max-w-full max-h-full rounded-md" alt="Apple Watch" /> : <img src={c.image} class="w-16 md:w-32 max-w-full max-h-full rounded-md" alt={c.name} />
                      }
                    </td>
          ...
          </table> : <div className='text-center text-xl m-10'>No Categories In Trending</div>
        }
      </div>
    </div>
  )
